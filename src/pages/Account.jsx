...
function Account() {
    ...
    const HandleCopy = async () => {
        try {
            await navigator.clipboard.writeText(`http://localhost:5173/linkTreeClone/${mainData?.username}`);
            toast.dismiss();
            ...
            <div className="flex flex-col items-center gap-5 py-28">
                <span className="">
                    ...
                    <span className="flex items-center gap-5">
                        <p className="bg-stone-200 w-60 rounded-sm overflow-x-scroll py-2 px-2 ">{`http://localhost:5173/linkTreeClone/${mainData?.username}`}</p>
